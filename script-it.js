...
function playAudio() {
    if (currentVocab?.italienisch) {
        ...
        const selectedVoice = voices.find(voice => 
            voice.lang === 'it-IT' &&
            (preferredVoiceGender === 'male' ? voice.name.toLowerCase().includes('male') : !voice.name.toLowerCase().includes('male'))
        );

        if (!selectedVoice) {
            utterance.voice = voices.find(voice => voice.lang === 'it-IT') || voices[0];
        } else {
            utterance.voice = selectedVoice;
        }

        if (utterance.voice) {
            console.log(`✅ Stimme ausgewählt: ${utterance.voice.name} (${utterance.voice.lang})`);
        } else {
            console.warn("⚠️ Keine Stimmen verfügbar, Standardsprache wird verwendet.");
        }
...
function markWrong() {
    console.log("❌ Falsche Antwort markiert.");
    if (currentVocab) {
...
function markCorrect() {
    console.log("✅ Richtige Antwort markiert.");
    if (currentVocab) {
...
function selectNextVocab() {
    ...
    if (mainVocabIndex < vocab.length) {
        ...
        mainVocabIndex++;
        console.log(`🎲 Nächste Vokabel aus Hauptliste: ${currentVocab.italienisch}`);
        showNewQuestion();
    } else {
